...
const getAllTasks = async (req, res) => {
  ...
  try {
    ...
    if (response) {
      ...
      res
        .status(500)
        .json(response.error || "Some error occurred while getting task data.");
    }
  } catch (err) {
    console.log(err);
  }
  ...
  try {
    ...
    if (response) {
      ...
      res
        .status(500)
        .json(
          response.error || "Some error occurred while getting the task data."
        );
    }
  } catch (err) {
    console.log(err);
...
const createTask = async (req, res) => {
  ...
  try {
    ...
    if (response) {
      ...
      res
        .status(500)
        .json(response.error || "Some error occurred while creating the task.");
    }
  } catch (err) {
    console.log(err);
  }
...
const deleteTask = async (req, res) => {
  ...
  try {
    ...
    if (response.deletedCount > 0) {
      ...
      res
        .status(500)
        .json(response.error || "Some error occurred while deleting the task.");
    }
  } catch (err) {
    console.log(err);
  }
